...
import { lookupGuest, lookupExistingRsvps, saveRsvps, notifyGoogleSheets } from "../api/rsvp"
import 'react-phone-number-input/style.css'
import "../App.css"


const EVENTS = [
    { key: "ganeshPooja",      label: "Ganesh Pooja & Haldi", date: "June 3 — Thursday" },
...
const EVENT_DAYS = [
    {
        key: "june3", label: "June 3", weekday: "Thursday",
        events: [
    ...
    {
        key: "june4", label: "June 4", weekday: "Friday",
        events: [
    ...
    {
        key: "june5", label: "June 5", weekday: "Saturday",
        events: [
...
const Divider = ({ large = false }) => (
    <div className="flex items-center justify-center gap-2 md:gap-3">
        <span className={`h-px bg-[#691700] ${large ? "w-12 md:w-16" : "w-8 md:w-12"}`}></span>
...
const ModalStep1 = ({ firstName, lastName, phoneNumber, error, onFirstNameChange, onLastNameChange, onPhoneChange, onContinue, onClose }) => (
    <>
        ...
        {error && <p className="font-prata text-[#691700] text-sm text-center">{error}</p>}
        <div className="flex gap-3 w-full">
            <SecondaryButton onClick={onClose} className="flex-1">Cancel</SecondaryButton>
            <PrimaryButton onClick={onContinue} disabled={!firstName.trim() || !lastName.trim()} className="flex-1">
                Continue
            </PrimaryButton>
        </div>
    </>
...
const ModalStep2 = ({ firstName, lastName, onModify, onClose }) => {
    ...
    <>
        ...
        <div className="flex flex-col gap-3 w-full">
            <PrimaryButton onClick={() => window.open(HOTEL_URL, "_blank", "noopener,noreferrer")} className="w-full">
                Book Hotel
            </PrimaryButton>
            <SecondaryButton onClick={onModify} className="w-full">Modify RSVP</SecondaryButton>
            <button onClick={onClose} className="w-full font-prata text-sm text-[#888] px-3 py-1 cursor-pointer hover:text-[#691700] transition-colors">
                Close
            </button>
        </div>
    </>
...
const ModalStep3 = ({ partyMembers, attendance, dietary, error, submitting, onAttendanceChange, onDietaryChange, onSubmit, onClose }) => (
    <>
        <h3 className="font-prata text-2xl text-[#691700] text-center">Party RSVP</h3>
        <p className="font-prata text-[#5a5a5a] text-sm text-center leading-relaxed">
            ...
            {partyMembers.map(member => (
                <div key={member} className="flex flex-col gap-4">
                        ...
                        {EVENTS.map(event => (
                            <div key={event.key} className="flex flex-col gap-1.5">
                                <div className="flex flex-col">
                                    <span className="font-prata text-sm text-[#691700]">{event.label}</span>
                                    <span className="font-prata text-xs text-[#888]">{event.date}</span>
                                </div>
                                <AttendanceToggle
                    ...
                    <div className="flex flex-col gap-1.5">
                        <label className="font-prata text-sm text-[#1a1a1a]">Dietary Restrictions</label>
                        <textarea
                            rows={2}
...
const ModalConfirmation = ({ onClose }) => (
    <>
        ...
        <div className="flex gap-3 w-full">
            <SecondaryButton onClick={onClose} className="flex-1">Close</SecondaryButton>
            <PrimaryButton onClick={() => window.open(HOTEL_URL, "_blank", "noopener,noreferrer")} className="flex-1">
                Book Hotel
            </PrimaryButton>
        </div>
    </>
...
const WeddingLogistics = () => {
    ...
    const handleClose = () => { setShowModal(false); resetModal(); };
    ...
    const handleLookup = async () => {
        ...
        const members = [self, ...extras.map(m => m.toLowerCase()).filter(m => m !== self)];
        ...
        members.forEach(member => {
            ...
            if (!prior) return;
            EVENTS.forEach(e => {
                if (prior.events?.[e.key] !== undefined) att[member][e.key] = prior.events[e.key];
            });
            diet[member] = prior.dietaryRestrictions ?? "";
        });

        setExistingRsvpIds(Object.fromEntries(
    ...
    const handleAttendanceChange = (member, eventKey, value) => {
        setAttendance(prev => ({ ...prev, [member]: { ...prev[member], [eventKey]: value } }));
    };

    const allAnswered = () =>
        partyMembers.every(m => EVENTS.every(e => attendance[m]?.[e.key] !== null && attendance[m]?.[e.key] !== undefined));
    ...
    const handleSubmit = async () => {
        ...
        const formattedPhone = parsed ? `+${parsed.countryCallingCode} ${parsed.nationalNumber}` : phoneNumber;

        const submittedBy = `${firstName.trim()} ${lastName.trim()}`.toLowerCase();
        const memberRsvps = partyMembers.map(m => ({
            name:                m,
            events:              EVENTS.reduce((acc, e) => ({ ...acc, [e.key]: attendance[m][e.key] }), {}),
    ...
    return (
        <div className="min-h-full bg-[#faf0e6] py-14 px-4">
                    ...
                    >
                        ...
                        {step === 3 && !submitted && (
                            <ModalStep3
                                ...
                                onDietaryChange={(member, val) => setDietary(prev => ({ ...prev, [member]: val }))}
            ...
            <section className="max-w-2xl md:max-w-3xl mx-auto px-4 py-6 text-center">
                <h2 className="font-prata text-3xl md:text-4xl text-[#4a4a4a] mt-1 md:mt-2 mb-2">RSVP</h2>
                <div className="mb-2 md:mb-4"><Divider /></div>
                <div className="font-prata text-[#5a5a5a] text-md md:text-lg mt-2 space-y-2.5 md:space-y-3">
                    ...
                    <p className="leading-relaxed">Please RSVP below so we know who from your party will be joining us for each event.</p>
            ...
            <section className="max-w-2xl md:max-w-3xl mx-auto px-4 py-6 mb-10">
                <h2 className="font-prata text-3xl md:text-4xl text-[#4a4a4a] text-center mb-2">Events</h2>
                <div className="mb-6"><Divider /></div>

                {EVENT_DAYS.map((day, i) => (
                    <div key={day.key} className={i < EVENT_DAYS.length - 1 ? "border-b border-[#691700]/15" : ""}>
                        <button
                        ...
                        {openDays.has(day.key) && (
                            <div className="flex flex-col gap-6 pb-8">
                                {day.events.map((event, j) => <EventCard key={j} {...event} />)}
                            </div>
                        )}
                    </div>
                ))}
