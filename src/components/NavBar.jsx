...
const NavBar = ({ showPlay, setScrollHidesControls }) => {
  ...
  }, [isMenuOpen, location.pathname, setScrollHidesControls]);

  return (
    <header
      className={`w-full py-2 md:py-3.25 px-3.25 sticky top-0 z-50 bg-gray-100 border-b border-[#691700] transition-transform duration-300 ${
        isVisible ? "translate-y-0" : "-translate-y-full md:translate-y-0"
      }`}
    >
      <nav
        className={`hidden md:flex justify-end items-center gap-2 ${showPlay ? "pr-52" : "pr-26"}`}
      >
        <Link
          to="/"
